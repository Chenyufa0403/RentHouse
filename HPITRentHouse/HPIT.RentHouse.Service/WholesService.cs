...
{
    public class WholesService : IWholesService
    {
        /// <summary>
        /// 查询房屋
        /// </summary>
        /// <returns></returns>
        public List<WholesDTO> GetList()
        {
            var db = new RentHouseEntity();
            BaseService<T_Houses> bs = new BaseService<T_Houses>(db);
            var list = bs.GetList(h => true).Select(h => new WholesDTO
            {
                Address = h.Address,
                Area = h.Area,
        ...
        /// <returns></returns>
        public List<WholesDTO> GetPageList(int start, int length, string name, ref int count)
        {
            var db = new RentHouseEntity();
            ...
            var result = list.Select(h => new WholesDTO
            {
                Address = h.Address,
                Area = h.Area,
                StatusId = h.StatusId.ToString(),
        ...
        public AjaxResult Add(WholesAddDTO admin)
        {
        ...
        public AjaxResult Edit(WholesAddDTO admin)
        {
            throw new NotImplementedException();
        }
